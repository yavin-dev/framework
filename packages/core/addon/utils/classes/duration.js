...
const VALID_ISO_DATE_DURATION = new RegExp(
  `^P([1-9]\\d*)([${Object.keys(DATE_UNITS).join('')}]$)`
);

// Regex for parsing iso time duration strings
const VALID_ISO_TIME_DURATION = new RegExp(
  `^PT([1-9]\\d*)([${Object.keys(TIME_UNITS).join('')}]$)`
);
