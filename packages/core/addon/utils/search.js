...
export function searchRecords(records, query, searchField) {
  ...
  for (let i = 0; i < records.length; i++) {
    let record = records.objectAt(i),
      relevance = getPartialMatchWeight(
        get(record, searchField).toLowerCase(),
        query
      );
...
export function searchDimensionRecords(records, query, resultLimit, page) {
  ...
  for (let i = 0; i < get(records, 'length'); i++) {
    ...
    let descriptionMatchWeight = getPartialMatchWeight(
        get(record, 'description').toLowerCase(),
        query.toLowerCase()
      ),
      idMatchWeight = getExactMatchWeight(
        get(record, 'id').toLowerCase(),
        query.toLowerCase()
      ),
