...
test('it returns true with rejected promise with forbidden error', function(assert) {
  assert.expect(4);
  assert.ok(
    isForbidden(new ForbiddenError({})),
    'returns true if forbidden error'
  );
  assert.notOk(
    isForbidden({ error: 'Not Found' }),
    'returns false for general errors'
  );
  assert.notOk(
    isForbidden(new BadRequestError({})),
    'returns false for bad requests'
  );
  assert.notOk(
    isForbidden(new ServerError({})),
    'returns false for server errors'
  );
});
