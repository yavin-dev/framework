...
test('subtractDurationFromDate', function(assert) {
  ...
  assert.notEqual(
    date,
    result,
    'subtractDurationFromDate does not mutate the date param object'
  );
...
test('computeStartOfInterval', function(assert) {
  ...
  let startOfInterval = DurationUtils.computeStartOfInterval(
    endDate,
    Duration.all(),
    'week'
  );
  ...
  expectedDate = moment(
    DateUtils.getFirstDayEpochIsoDateTimePeriod(
      'month',
      DateUtils.PARAM_DATE_FORMAT_STRING
    ),
    DateUtils.PARAM_DATE_FORMAT_STRING
