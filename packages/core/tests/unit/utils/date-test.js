...
test('getDatesForInterval', function(assert){
  ...
  assert.deepEqual(dates.map(date => date.format('D-M-Y')),
    [
      '4-9-2017',
      '11-9-2017',
      '18-9-2017',
...
test('getFirstDayOfPrevIsoDateTimePeriod - date format provided', function(assert){
  ...
  var expectedDate1 = moment().subtract(1, 'week').startOf('isoweek').format(dateFormat);
  assert.equal(DateUtils.getFirstDayOfPrevIsoDateTimePeriod('week', dateFormat),
    expectedDate1,
    `getFirstDayOfPrevIsoDateTimePeriod should return: ${expectedDate1}`);

  var expectedDate2 = moment().subtract(1, 'month').startOf('month').format(dateFormat);
  assert.equal(DateUtils.getFirstDayOfPrevIsoDateTimePeriod('month', dateFormat),
    expectedDate2,
    `getFirstDayOfPrevIsoDateTimePeriod should return: ${expectedDate2}`);

  var expectedDate3 = moment().subtract(1, 'year').startOf('year').format(dateFormat);
  assert.equal(DateUtils.getFirstDayOfPrevIsoDateTimePeriod('year', dateFormat),
    expectedDate3,
    `getFirstDayOfPrevIsoDateTimePeriod should return: ${expectedDate3}`);

  var expectedDate4 = moment().subtract(1, 'day').startOf('day').format(dateFormat);
  assert.equal(DateUtils.getFirstDayOfPrevIsoDateTimePeriod('day', dateFormat),
    expectedDate4,
...
test('getLastDayOfPrevIsoDateTimePeriod - unit tests', function(assert){
  ...
  let expectedDate1 = moment().startOf('isoweek').subtract(1, 'day').format(dateFormat);
  assert.equal(DateUtils.getLastDayOfPrevIsoDateTimePeriod('week', dateFormat),
    expectedDate1,
    'getLastDayOfPrevIsoDateTimePeriod should return: ' + expectedDate1);

  let expectedDate2 = moment().startOf('month').subtract(1, 'day').format(dateFormat);
  assert.equal(DateUtils.getLastDayOfPrevIsoDateTimePeriod('month', dateFormat),
    expectedDate2,
    'getLastDayOfPrevIsoDateTimePeriod should return: ' + expectedDate2);

  let expectedDate3 = moment().startOf('year').subtract(1, 'day').format(dateFormat);
  assert.equal(DateUtils.getLastDayOfPrevIsoDateTimePeriod('year', dateFormat),
    expectedDate3,
    'getFirstDayOfPrevIsoDateTimePeriod should return: ' + expectedDate3);

  let expectedDate4 = moment().startOf('day').subtract(1, 'day').format(dateFormat);
  assert.equal(DateUtils.getLastDayOfPrevIsoDateTimePeriod('day', dateFormat), expectedDate4, 'getFirstDayOfPrevIsoDateTimePeriod should return: ' + expectedDate4);
});
...
test('getFirstDayEpochIsoDateTimePeriod - Epoc date as mid of DateTimePeriod', function(assert){
  ...
  assert.equal(DateUtils.getFirstDayEpochIsoDateTimePeriod('week', dateFormat),
    expectedEpocDate1,
  ...
  assert.equal(DateUtils.getFirstDayEpochIsoDateTimePeriod('month', dateFormat),
    expectedEpocDate2,
  ...
  assert.equal(DateUtils.getFirstDayEpochIsoDateTimePeriod('year', dateFormat),
    expectedEpocDate3,
...
test('getFirstDayEpochIsoDateTimePeriod - Epoc date as start of DateTimePeriod', function(assert){
  ...
  assert.equal(DateUtils.getFirstDayEpochIsoDateTimePeriod('day', dateFormat),
    expectedEpocDate1,
    'getFirstDayEpochIsoDateTimePeriod should return: ' + expectedEpocDate1);
...
test('getFirstDayOfIsoDateTimePeriod - unit tests', function(assert){
  ...
  assert.throws(() => DateUtils.getFirstDayOfIsoDateTimePeriod(undefined, 'week', dateFormat),
    'Threw an error as expected');

  let expectedDate1 = moment("2014-10-13").format(dateFormat);
  assert.equal(DateUtils.getFirstDayOfIsoDateTimePeriod(moment("2014-10-15"), 'week', dateFormat),
    expectedDate1,
    'getFirstDayOfIsoDateTimePeriod returned: ' + expectedDate1 + ' as expected');

  assert.throws(() => DateUtils.getFirstDayOfIsoDateTimePeriod(),
    'Threw an error as expected');

  let expectedDate2 = moment().startOf('isoweek').format(dateFormat);
  assert.equal(DateUtils.getFirstDayOfIsoDateTimePeriod(moment(), 'week'),
    expectedDate2,
