...
test('isEqual', function(assert) {
  ...
  assert.ok(
    interval.isEqual(interval),
    'Interval equals one with matching start/end'
  );
});
...
test('diffForTimePeriod', function(assert) {
  ...
  assert.equal(
    new Interval(moment('2014-10-10'), moment('2015-10-10')).diffForTimePeriod(
      'day'
    ),
    365,
  ...
  assert.equal(
    new Interval(
      moment('2015-11-09 00:00:00.000'),
      moment('2015-11-10 10:00:00.000')
    ).diffForTimePeriod('hour'),
    34,
    'Interval has 34 hours for a 1 day 10 hour time period'
  );

  assert.equal(
    new Interval(moment('2015-11-09'), moment('2015-11-10')).diffForTimePeriod(
      'day'
    ),
    1,
  ...
  assert.equal(
    new Interval(
      moment('2015-11-10 10:00:00.000'),
      moment('2015-11-10 11:00:00.000')
    ).diffForTimePeriod('hour'),
    1,
  ...
  assert.equal(
    new Interval(
      moment('2015-11-10 10:00:00.000'),
      moment('2015-11-13 11:00:00.000')
    ).diffForTimePeriod('all'),
    1,
...
test('asMoments', function(assert) {
  ...
  assert.ok(
    moments.start.isSame(sevenDaysAgo, 'day'),
    'Duration is correctly subtracted from end'
  );

  assert.ok(
    moments.end.isSame(current, 'day'),
  ...
  assert.ok(
    moments.start.isSame(start) && moments.end.isSame(end),
...
test('asMomentsForTimePeriod', function(assert) {
  ...
  assert.ok(
    moments.start.isSame(start.startOf('isoweek')),
    'Start moment is at start of isoweek'
  );

  assert.ok(
    moments.end.isSame(end.startOf('isoweek')),
...
test('asMomentsForTimePeriod with current and next for time period', function(assert) {
  ...
  assert.ok(
    moments.end.isSame(expected),
    'Setting end to next will be computed correctly'
  );
});
...
test('asIntervalForTimePeriod', function(assert) {
  ...
  let start = moment('2014-10-10', FORMAT),
    ...
    newInterval = new Interval(start, end)
      .asIntervalForTimePeriod('week')
      .asMoments();

  assert.ok(
    newInterval.start.isSame(start.startOf('isoweek')),
    'Start moment is at start of isoweek'
  );

  assert.ok(
    newInterval.end.isSame(end.startOf('isoweek')),
    'End moment is at start of isoweek'
  );
});
...
test('asIntervalForTimePeriod - same start and end date', function(assert) {
  ...
  assert.equal(
    newInterval.start.format(FORMAT),
    '2017-10-09',
    'Start moment is at start of isoweek'
  );

  assert.equal(
    newInterval.end.format(FORMAT),
    '2017-10-16',
    'End moment is at start of isoweek'
  );
});
...
test('_stringFromProperty', function(assert) {
  ...
  assert.equal(
    Interval._stringFromProperty(new Duration('P7D')),
    'P7D',
    'Duration is converted to iso string'
  );

  assert.equal(
    Interval._stringFromProperty('current'),
    'current',
...
test('fromString', function(assert) {
  ...
  assert.equal(
    Interval.fromString('current'),
    'current',
    'Macro can be parsed from string'
  );

  assert.ok(
    Interval.fromString('P7D').isEqual(new Duration('P7D')),
    'Duration can be parsed from iso string'
  );

  assert.ok(
    Interval.fromString('2014-10-10 00:00:00.000').isSame(
      moment('2014-10-10', FORMAT)
    ),
...
test('_isAcceptedType', function(assert) {
  ...
  assert.ok(
    interval._isAcceptedType(new Duration('P7D')),
  ...
  assert.ok(
    interval._isAcceptedType(moment('2014-10-10', FORMAT)),
    'Moments are accepted'
  );

  assert.notOk(
    interval._isAcceptedType({ random: 'object' }),
...
test('isInterval', function(assert) {
  ...
  assert.ok(
    Interval.isInterval(new Interval('current', 'current')),
    'isInterval returns true for intervals'
  );

  assert.notOk(
    Interval.isInterval({ random: 'object' }),
