...
module('Unit | Utility | persistence error');

test('getApiErrMsg', function (assert) {
  assert.expect(3);

  assert.equal(getApiErrMsg({}, 'default'),
    'default',
    'getApiErrMsg returns the default message if no detail');

  let detail = ['Object details'];
  assert.equal(getApiErrMsg({ detail }),
    detail[0],
