import { getPartialMatchWeight, getExactMatchWeight, searchRecords, searchDimensionRecords } from 'navi-core/utils/search';
...
test('getPartialMatchWeight', function(assert){
  ...
  assert.equal(getPartialMatchWeight('heavy green character', 'heavy yellow character'),
    undefined,
    'No match weight when not all words are found');

  assert.equal(getPartialMatchWeight('heavy green character', 'character heavy'),
    7,
...
test('getExactMatchWeight', function(assert){
  ...
  assert.equal(getExactMatchWeight('158', 'karts'),
    undefined,
    'No match weight when query does not match');
  assert.equal(getExactMatchWeight('15897', '158'),
    3,
...
test('searchRecords', function(assert) {
  ...
  assert.deepEqual(searchRecords(records, 'Bike', 'description'), [
    {
      "description": "All Bikes",
      "id": "bike"
    },
  ...
  assert.deepEqual(searchRecords(records, '123', 'id'), [
    {
      "description": "Standard Kart",
      "id": "123"
    },
...
test('searchDimensionRecords', function(assert){
  ...
  assert.deepEqual(results.mapBy('record.id'), ['123', '123456',
    '1234567'],
