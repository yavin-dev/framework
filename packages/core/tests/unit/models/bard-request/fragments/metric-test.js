...
test('Model using the Metric Fragment', function(assert) {
  ...
  assert.equal(mockModel.get('metrics').objectAt(0).get('canonicalName'),
    'pageViews',
    'The property metric is set as `Page Views` using the setter');
...
test('Model metric that has parameters', function(assert) {
  ...
  return run(() => {
      ...
      data: {
        ...
        attributes: {
          metrics: [
            ...
            {metric: 'totalClicks', parameters: {c: 'one', a: 'two', b: 'three'}}
          ]
        }
      }
    });
    ...
    assert.equal(metricModel.get('parameters.currency'),
      'USD',
      'Should see USD as currency parameter');
    assert.equal(metricModel.get('canonicalName'),
      'revenue(currency=USD)',
      'Canonical name should render correctly');

    let multiParamMetricModel = mockModel.get('metrics').objectAt(1);
    assert.deepEqual(Object.keys(multiParamMetricModel.get('parameters')),
      ['c', 'a', 'b'],
      'Check that keys are not alpha ordered');
    assert.equal(multiParamMetricModel.get('canonicalName'),
...
test('Validations', function(assert) {
  ...
  let metric = run(() => Store.peekRecord('fragments-mock', 1).get('metrics').objectAt(0));
