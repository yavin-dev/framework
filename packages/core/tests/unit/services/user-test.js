...
test('getUser - invoked without userId param', function(assert) {
  ...
  Ember.run(() => {
    ...
    assert.equal(service.getUser(),
      userModel,
      `getUser return model for registered logged-in "${NaviUser}" user`);
  });
});
...
test('getUser - invoked with userId param', function(assert) {
  ...
  assert.equal(service.getUser(naviUser),
    null,
    'getUser returns null for non registered');
  ...
  Ember.run(() => {
    ...
    assert.equal(service.getUser(naviUser),
      userModel,
      `getUser return model for registered "${naviUser}" user`);
  });
});
...
test('findUser - invoked without userId param', function(assert) {
  ...
  return Ember.run(() => {
    return service.findUser().then(user => {
      ...
      assert.equal(user,
        userModel,
        `findUser returns model for registered logged-in "${NaviUser}" user`);

      config.navi.user = 'unregistered_user';
      return service.findUser().catch(() => {
        assert.ok(true,
          'findUser returns an error promise when an unregistered user is fetched');
      });
    });
  });
});
...
test('findUser - invoked with userId param', function(assert) {
  ...
  return Ember.run(() => {
    return service.findUser(userId).then(user => {
      ...
      assert.ok(user,
        'findUser returns a non empty user model');
...
test('register - unregistered logged-in user', function(assert) {
  ...
  return Ember.run(() => {
    ...
    assert.notOk(Store.peekRecord('user', naviUser),
    ...
    return service.register().then(user => {
      ...
      assert.ok(user,
        'register returns a non empty model as expected');

      assert.equal(user,
        userModel,
        `register returns model for "${naviUser}" user`);
    });
  });
});
...
test('register - handle server error', function(assert) {
  ...
  return Ember.run(() => {
    return service.register().catch(() => {
      assert.ok(true,
        'register return an error promise when server throws an error');
    });
  });
});
...
test('findOrRegister - unregistered user', function(assert) {
  ...
  return Ember.run(() => {
    ...
    return service.findOrRegister().then(user => {
      ...
      assert.equal(user,
        userModel,
        'findOrRegister registers an unregistered user and return model as expected');
    });
  });
});
...
test('findOrRegister - registered user not present in store', function(assert) {
  ...
  return Ember.run(() => {
    ...
    assert.equal(Store.peekRecord('user', NaviUser),
      null,
      `"${NaviUser}" user is initially not present in the store`);
    ...
    return service.findOrRegister().then(user => {
      ...
      assert.ok(user,
        'findOrRegister returns a non empty user model');

      assert.equal(user,
        userModel,
        'findOrRegister returns model for a registered user');
    });
  });
});
