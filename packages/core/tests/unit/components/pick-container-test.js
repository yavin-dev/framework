...
test('Registering', function(assert) {
  ...
  assert.deepEqual(component.get('_registered'),
    {},
    'No components are registered by default');
  ...
  assert.throws(() => { component.register('myComponent', 8); },
...
test('One way selection binding', function(assert) {
  ...
  assert.equal(component.get('_editableSelection'),
    1,
    '_editableSelection starts equal to selection');

  component.set('_editableSelection', 2);
  assert.notEqual(component.get('_editableSelection'),
  ...
  assert.equal(component.get('_editableSelection'),
    3,
    'Changing selection updates _editableSelection');
});
...
test('One way selection binding with complex objects', function(assert) {
  ...
  assert.deepEqual(component.get('selection'),
    [1, 2, 3],
    'Selection is unaffected by changes to the underlying object');

  assert.deepEqual(component.get('_editableSelection'),
    [1, 2, 3, 4],
    '_editableSelection can be changed');
});
...
test('Selection change overwrites staged change', function(assert) {
  ...
  assert.equal(component.get('_editableSelection'),
    2,
    'Staged changes are visible when selection remains the same');
  ...
  assert.equal(component.get('_editableSelection'),
    3,
    'Setting selection overwrites staged changes');
});
...
test('applyChanges does not reset form', function(assert) {
  ...
  assert.equal(component.get('selection'),
    outsideSelection,
    'Selection is unaffected by applied changes');

  assert.equal(component.get('_editableSelection'),
    insideSelection,
    'Form state is not reset when applying changes');
});
...
test('Get staged selection', function(assert) {
  ...
  assert.equal(component.getStagedSelection(),
    3,
