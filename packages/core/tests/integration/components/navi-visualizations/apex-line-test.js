...
module('Integration | Component | Apex-Line', function(hooks) {
  ...
  test('apex-line chart legend renders', async function(assert) {
  ...
  test('apex-line chart x-axis dates render', async function(assert) {
    await this.render(TEMPLATE);
    let legendText = [];
  ...
  test('apex line chart lines render', async function(assert) {
    await this.render(TEMPLATE);
    assert.dom('.apexcharts-line-series').exists();
    assert.dom('g.apexcharts-series>path.apexcharts-line').exists({ count: expectedInfo.series.length });
  });
});
