...
module('Integration | Component | Config - Color-Change', function(hooks) {
  ...
  test('color-change label selector renders', async function(assert) {
    assert.expect(4);
    await render(TEMPLATE);
    assert.dom('.color-change-config__label-select').exists();
    assert.dom('.color-change-config__label-select__label').hasText('Label:');
    assert.dom('.color-change-config__label-select__selector').exists();
    await clickTrigger('.color-change-config__label-select__selector');
    assert.dom('.ember-power-select-option').exists({ count: RESPONSE.length });
  });

  test('color-change color selector renders', async function(assert) {
    assert.expect(3);
    await render(TEMPLATE);
    assert.dom('.color-change-config__color-select').exists();
    assert.dom('.color-change-config__color-select__label').hasText('Color:');
    assert.dom('.color-change-config__color-select__input').exists();
  });
});
