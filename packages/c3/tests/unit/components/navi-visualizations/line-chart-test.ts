...
import { GROUP } from '@yavin/c3/chart-builders/date-time';
...
module('Unit | Component | line chart', function (hooks) {
  ...
  test('line chart styles', function (assert) {
    ...
    assert.strictEqual(component.config.data.type, 'line');

    //@ts-expect-error
    set(component.args, 'options', { style: { curve: 'line', area: true } });
    assert.strictEqual(component.config.data.type, 'area', 'default of line returns as configured');

    //@ts-expect-error
    set(component.args, 'options', { style: { curve: 'spline', area: false } });
    assert.strictEqual(component.config.data.type, 'spline', 'adding spline passes a spline config');

    //@ts-expect-error
    set(component.args, 'options', { style: { curve: 'spline', area: true } });
    assert.strictEqual(component.config.data.type, 'area-spline', 'spline with area true returns a area spline config');

    //@ts-expect-error
    set(component.args, 'options', { style: { curve: 'step', area: false } });
    assert.strictEqual(component.config.data.type, 'step', 'step returns a step config');

    //@ts-expect-error
    set(component.args, 'options', { style: { curve: 'step', area: true } });
    assert.strictEqual(component.config.data.type, 'area-step', 'step with area true returns a area step config');

    //@ts-expect-error
    set(component.args, 'options', { style: { curve: 'moose', area: false } });
    assert.strictEqual(component.config.data.type, 'line', 'bad config uses default line');
  });
