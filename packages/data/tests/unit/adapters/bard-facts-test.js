...
test('_buildDimensionsPath', function(assert) {
  ...
  assert.equal(Adapter._buildDimensionsPath(singleDim),
    '/d1',
  ...
  assert.equal(Adapter._buildDimensionsPath(manyDims),
    '/d1/d2',
    '_buildDimensionsPath built the correct string for many dimensions');

  let noDims = {};
  assert.equal(Adapter._buildDimensionsPath(noDims),
    '',
...
test('_buildDateTimeParam', function(assert) {
  ...
  assert.equal(Adapter._buildDateTimeParam(singleInterval),
    'start/end',
  ...
  assert.equal(Adapter._buildDateTimeParam(manyIntervals),
...
test('_buildMetricsParam', function(assert) {
  ...
  assert.equal(Adapter._buildMetricsParam(singleMetric),
    'm1',
  ...
  assert.equal(Adapter._buildMetricsParam(manyMetrics),
    'm1,m2',
...
test('_buildFiltersParam', function(assert) {
  ...
  let singleFilter = {
    filters: [
      { dimension: 'd1', field: 'desc', operator: 'in', values: ['v1', 'v2'] }
    ]
  };
  assert.equal(Adapter._buildFiltersParam(singleFilter),
    'd1|desc-in[v1,v2]',
  ...
  let manyFilters = {
    filters: [
      { dimension: 'd1', field: 'desc', operator: 'in',    values: ['v1', 'v2'] },
      { dimension: 'd2', field: 'id',   operator: 'notin', values: ['v3', 'v4'] }
    ]
  };
  assert.equal(Adapter._buildFiltersParam(manyFilters),
    'd1|desc-in[v1,v2],d2|id-notin[v3,v4]',
    '_buildFiltersParam built the correct string for many filters');

  let noFilters = {};
  assert.equal(Adapter._buildFiltersParam(noFilters),
    undefined,
    '_buildFiltersParam returns undefined with no filters');

  let emptyFilters = {filters: []};
  assert.equal(Adapter._buildFiltersParam(emptyFilters),
    undefined,
...
test('_buildSortParam', function(assert) {
  ...
  assert.equal(Adapter._buildSortParam(singleSort),
    'm1|asc',
  ...
  assert.equal(Adapter._buildSortParam(sortNoDirection),
    'm1|desc',
  ...
  assert.equal(Adapter._buildSortParam(manySorts),
    'm1|asc,m2|asc',
  ...
  assert.equal(Adapter._buildSortParam(noSorts),
    undefined,
    '_buildSortParam returns undefined with no sort');

  let emptySorts = {sort: []};
  assert.equal(Adapter._buildSortParam(emptySorts),
    undefined,
    '_buildSortParam returns undefined with empty sort');

  let invalidSort = {sort: [
    {metric: 'valid1'},
    {metric: 'valid2', direction: 'asc'},
    {metric: 'valid3', direction: 'desc'},
    {metric: 'invalid', direction: 'foo'},
  ]};
  assert.throws(() => { Adapter._buildSortParam(invalidSort); },
...
test('_buildHavingParam', function(assert) {
  ...
  assert.equal(Adapter._buildHavingParam(singleHaving),
    'm1-gt[0]',
  ...
  assert.equal(Adapter._buildHavingParam(manyHavings),
    'm1-gt[0],m2-lte[10]',
    '_buildHavingParam built the correct string for multiple having');

  let noHavings = {};
  assert.equal(Adapter._buildHavingParam(noHavings),
    undefined,
    '_buildHavingParam returns undefined with no having');

  let emptyHavings = {having: []};
  assert.equal(Adapter._buildHavingParam(emptyHavings),
    undefined,
...
test('_buildURLPath', function(assert) {
  ...
  assert.equal(Adapter._buildURLPath(TestRequest),
...
test('_buildQuery', function(assert) {
  ...
  let noFiltersAndNoHavings = Ember.$.extend({}, TestRequest, {filters: [], having: []});
  ...
  assert.deepEqual(Adapter._buildQuery(TestRequest, {format: 'jsonApi'}),
    {
      dateTime: '2015-01-03/2015-01-04',
      filters:  'd3|id-in[v1,v2],d4|id-in[v3,v4],d5|id-notnull[""]',
      metrics:  'm1,m2,r(p=123)',
  ...
  let sortRequest = Ember.$.extend({}, TestRequest, {sort: [{metric: 'm1'}]});
...
test('_buildQuery with catchAll Query Params', function(assert) {
  ...
  assert.deepEqual(Adapter._buildQuery(TestRequest, options),
    {
      dateTime: '2015-01-03/2015-01-04',
      filters:  'd3|id-in[v1,v2],d4|id-in[v3,v4],d5|id-notnull[""]',
      metrics:  'm1,m2,r(p=123)',
...
test('_buildQuery with pagination', function(assert) {
  ...
  assert.deepEqual(Adapter._buildQuery(TestRequest, { page: 1, perPage: 100 }),
    {
      dateTime: '2015-01-03/2015-01-04',
      filters:  'd3|id-in[v1,v2],d4|id-in[v3,v4],d5|id-notnull[""]',
      metrics:  'm1,m2,r(p=123)',
  ...
  assert.deepEqual(Adapter._buildQuery(TestRequest, { format: 'csv' }),
    {
      dateTime: '2015-01-03/2015-01-04',
      filters:  'd3|id-in[v1,v2],d4|id-in[v3,v4],d5|id-notnull[""]',
      metrics:  'm1,m2,r(p=123)',
...
test('fetchDataForRequest with pagination options', function(assert) {
  assert.expect(1);
  return Adapter.fetchDataForRequest(TestRequest, {page: 1, perPage: 100}).then(function(result){
    return assert.deepEqual(result,
      {
        rows: [
          {
...
test('fetchDataForRequest with custom headers', function(assert) {
  ...
  Server.get(`${HOST}/v1/data/table1/grain1/d1/d2/`, (request) => {
    assert.equal(request.requestHeaders.foo,
      'bar',
      'custom header `foo` was sent with the request');

    assert.equal(request.requestHeaders.baz,
      'qux',
      'custom header `baz` was sent with the request');
