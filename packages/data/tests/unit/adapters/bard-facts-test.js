...
moduleFor('adapter:bard-facts', 'Unit | Bard facts Adapter', {
  needs: ['service:ajax', 'service:request-decorator'],
...
test('_buildSortParam', function(assert) {
  ...
  let noSorts = {};
  assert.equal(
    Adapter._buildSortParam(noSorts),
    undefined,
  ...
  let emptySorts = { sort: [] };
  assert.equal(
    Adapter._buildSortParam(emptySorts),
    undefined,
...
test('_buildHavingParam', function(assert) {
  ...
  let noHavings = {};
  assert.equal(
    Adapter._buildHavingParam(noHavings),
    undefined,
...
test('_buildQuery', function(assert) {
  ...
  let noFiltersAndNoHavings = Ember.$.extend({}, TestRequest, {
    filters: [],
    having: []
  });
  assert.deepEqual(
  ...
  let sortRequest = Ember.$.extend({}, TestRequest, {
    sort: [{ metric: 'm1' }]
  });
  assert.deepEqual(
  ...
  let havingRequest = Ember.$.extend({}, TestRequest, {
...
test('urlForFindQuery', function(assert) {
  ...
  let noFiltersNoHavings = Ember.$.extend({}, TestRequest, {
    filters: null,
    having: null
  });
  assert.equal(
...
test('fetchDataForRequest with client id options', function(assert) {
  ...
  Server.get(`${HOST}/v1/data/table1/grain1/d1/d2/`, request => {
    assert.equal(
      request.requestHeaders.clientid,
      'UI',
