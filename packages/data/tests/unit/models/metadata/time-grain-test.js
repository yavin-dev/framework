...
moduleFor('model:metadata/time-grain', 'Unit | Metadata Model | Time Grain', {
  needs: ['service:keg', 'model:metadata/dimension', 'model:metadata/metric'],
...
test('metrics and dimensions', function(assert) {
  ...
  assert.deepEqual(
    ...
    [
      Keg.getById('metadata/dimension', 'age'),
      Keg.getById('metadata/dimension', 'gender')
    ],
