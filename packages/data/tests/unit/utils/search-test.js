...
test('getPartialMatchWeight', function(assert){
  ...
  assert.equal(SearchUtils.getPartialMatchWeight('heavy green character', 'kart weight'),
    undefined,
    'No match weight when query does not match');

  assert.equal(SearchUtils.getPartialMatchWeight('heavy green character', 'heavy yellow character'),
    undefined,
    'No match weight when not all words are found');

  assert.equal(SearchUtils.getPartialMatchWeight('heavy green character', 'character heavy'),
    7,
  ...
  assert.ok(weight1 > weight2,
    'Closer match has smaller match weight');
});

test('getExactMatchWeight', function(assert){
  assert.expect(3);

  assert.equal(SearchUtils.getExactMatchWeight('158', 'karts'),
    undefined,
    'No match weight when query does not match');

  assert.equal(SearchUtils.getExactMatchWeight('15897', '158'),
    3,
...
test('searchDimensionRecords', function(assert){
  ...
  var results = Ember.A(SearchUtils.searchDimensionRecords(records, 'Bike', 100));

  assert.equal(results[0].record.get('description'),
    'All Bikes',
    'First result is most relevant dimension');

  assert.equal(results[0].relevance,
    1,
  ...
  assert.equal(results.length,
    2,
    'Search results are correctly limited');

  results = Ember.A(SearchUtils.searchDimensionRecords(records, '123', 2, 2));
  assert.deepEqual(results.mapBy('record'),
    [records[2]],
  ...
  assert.deepEqual(results.mapBy('record.id'),
    ['123', '123456', '1234567'],
...
test('searchDimensionRecords - without description', function(assert){
  ...
  let results = Ember.A(SearchUtils.searchDimensionRecords(records, 'bike', 100));
