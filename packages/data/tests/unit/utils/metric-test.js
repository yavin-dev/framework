...
test('has parameters check', function(assert) {
  assert.expect(5);
  assert.equal(
    hasParameters({ metric: 'foo' }),
    false,
    'metric with no params'
  );

  assert.equal(
    hasParameters({ metric: 'foo', parameters: {} }),
    false,
    'metric with empty object params'
  );

  assert.equal(
    hasParameters({ metric: 'foo', parameters: null }),
    false,
    'metric with null object params'
  );

  assert.equal(
    hasParameters({ metric: 'foo', parameters: { p1: '100' } }),
    true,
    'metric with one param'
  );

  assert.equal(
    hasParameters({ metric: 'foo', parameters: { p1: '100', a: '12' } }),
    true,
    'multiple params'
  );
});
...
test('serialize parameters check', function(assert) {
  ...
  assert.equal(
    serializeParameters({ b: 1, c: 2, a: 3 }),
    'a=3,b=1,c=2',
    'metric with multiple parameters'
  );

  assert.equal(
    serializeParameters({ currency: 'USD' }),
    'currency=USD',
...
test('get canonicalized metric from alias', function(assert) {
  ...
  assert.equal(
    canonicalizeAlias('m1', aliasMap),
    'bar(pos=1)',
    'gets canonicalized metric from alias'
  );
  assert.equal(
    canonicalizeAlias('m3', aliasMap),
    'm3',
    "returns what's passed if not in alias map"
  );
  assert.equal(
    canonicalizeAlias('m1'),
    'm1',
    'returns alias without alias map'
  );
  assert.equal(
    canonicalizeAlias('m1', {}),
    'm1',
    'returns alias with empty alias map'
  );
});
