...
test('push inserts a record into the keg', function(assert) {
  ...
  assert.ok(
    pushedRecord instanceof RecordFactory.class,
    'push returns an instance of the given type'
  );
...
test('push can take an explicit modelFactory', function(assert) {
  ...
  assert.ok(
    pushedRecord instanceof RecordFactory.class,
    'push returns an object instance of the explicit type'
  );
});
...
test('all returns all records for a type', function(assert) {
  ...
  let unregisteredTypeRecords = Keg.pushMany(
    'unregisteredType',
