...
moduleFor('service:keg', 'Unit | Service | keg', {
  beforeEach() {
    Keg = this.subject();
    ...
    getOwner(this).register('model:record', Ember.Object.extend());
    RecordFactory = getOwner(this).factoryFor('model:record');
  }
});
...
test('_getFactoryForType returns factory for a model type', function(assert) {
  ...
  assert.equal(Keg._getFactoryForType('record'),
    RecordFactory.class,
    '_getFactoryForType returns factory of model type when type is registered');

  let testType = {};
  assert.equal(Keg._getFactoryForType(testType),
    testType,
...
test('_getRecordKegForType returns a keg for a type', function(assert) {
  ...
  assert.deepEqual(recordKeg.toArray(),
    [],
    '_getRecordKegForType returns an empty array when called the first time');

  //Mock a record insert
  recordKeg.pushObject({id: 1});

  assert.deepEqual(Keg._getRecordKegForType('record').toArray(),
    [{id: 1}],
...
test('_getIdIndexForType returns an id index for a type', function(assert) {
  ...
  assert.deepEqual(idIndex,
    {},
    '_getIdIndexForType returns an empty object when called the first time');

  //Mock a record insert
  idIndex[1] = {id: 1};

  assert.deepEqual(Keg._getIdIndexForType('record'),
    { 1: {id: 1} },
...
test('push inserts a record into the keg', function(assert) {
  ...
  assert.equal(get(pushedRecord, 'id'),
    get(Record1, 'id'),
    'The returned record has the correct id');

  assert.equal(get(pushedRecord, 'description'),
    get(Record1, 'description'),
    'The returned record has the correct description');

  assert.equal(get(Keg, 'recordKegs.record.firstObject'),
    pushedRecord,
    'The pushed record is registered in recordKeg');

  assert.deepEqual(get(Keg, 'idIndexes.record'),
    { 1: pushedRecord },
  ...
  assert.equal(pushedRecord,
    foundRecord,
    'The pushed record is the same as the found record');
});
...
test('pushing a record with an existing id update the record in the keg', function(assert) {
  ...
  assert.deepEqual(Keg.all('record'),
    [pushedRecord],
  ...
  assert.equal(get(fetchedRecord, 'description'),
    'updated',
...
test('pushMany inserts many records into the keg', function(assert) {
  ...
  assert.ok(pushedRecords.every(rec => { return rec instanceof RecordFactory.class; }), 'pushMany returns an array of instances of the given type');

  assert.deepEqual(pushedRecords.mapBy('id'),
    [1, 2],
    'pushed records have the correct ids');

  assert.deepEqual(pushedRecords.mapBy('description'),
    ['foo', 'bar'],
    'pushed records have the correct descriptions');

  let allRecords = Keg.all('record');
  pushedRecords.forEach((rec, idx) => {
    assert.equal(pushedRecords.get(idx),
      allRecords.get(idx),
      'The pushed records are the same as the fetched records');
  });

  assert.deepEqual(get(Keg, 'recordKegs.record').toArray(),
...
test('pushMany updates keg records when provided records have the same id', function(assert) {
  ...
  assert.equal(secondPush.get('firstObject'),
...
test('pushMany can take an explicit modelFactory', function(assert) {
  ...
  assert.ok(pushedRecords.every(rec =>  { return rec instanceof RecordFactory.class; }), 'pushMany returns an array of instances of the explicit type');
});

test('pushMany uses identifierField in factory if available', function(assert) {
  assert.expect(2);

  let recordFactory = getOwner(this).factoryFor('model:record').class;
  ...
  assert.equal(Keg.getById('record', 'foo'),
    pushedRecords.get(0),
...
test('getById can be used to find a record by id', function(assert) {
  ...
  assert.equal(get(foundRecord, 'id'),
    get(Record1, 'id'),
    'record has correct id');

  assert.equal(get(foundRecord, 'description'),
    get(Record1, 'description'),
    'record has correct description');

  let missingRecord = Keg.getById('record', 22);
  assert.equal(missingRecord,
    undefined,
    'getById returns undefined when given an id that is not present');

  let missingType = Keg.getById('nonRegisteredType', 22);
  assert.equal(missingType,
    undefined,
    'getById returns undefined when given a type that is not present');
});
...
test('getBy can be used to search by fields when an object is passed', function(assert) {
  ...
  assert.deepEqual(missingType,
    [],
    'getBy returns an empty array when given a type that is not present');
});
...
test('getBy can be used to search by fields when a function is passed', function(assert) {
  ...
  let foundRecords1 = Keg.getBy('record', rec => { return get(rec, 'id') <  2;} );
  assert.deepEqual(foundRecords1.mapBy('id'),
    [1],
    'getBy returns an array when a single record was found');

  let foundRecords2 = Keg.getBy('record', rec => { return get(rec, 'description') === 'bar';});
  assert.deepEqual(foundRecords2.mapBy('id'),
    [2,3],
...
test('all returns all records for a type', function(assert) {
  ...
  assert.deepEqual(Keg.all('record').toArray(),
    [],
  ...
  let unregisteredTypeRecords = Keg.pushMany('unregisteredType', [Record1, Record2, Record3], { modelFactory: Object });
  assert.deepEqual(Keg.all('unregisteredType').toArray(),
