...
test('all method', function(assert) {
  ...
  return Service.loadMetadata().then(() => {
    ...
    assert.throws(
      ...
      new Error(
        'Assertion Failed: Metadata must be loaded before the operation can be performed'
      ),
...
test('getMetaField', function(assert) {
  ...
  return Service.loadMetadata().then(() => {
    assert.equal(
      Service.getMetaField('metric', 'metricOne', 'longName'),
      'Metric One',
