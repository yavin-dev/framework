...
test('It formats metrics appropriately', function(assert) {
  assert.expect(6);
  assert.equal(metricFormat({ metric: 'revenue' }, 'Revenue'), 'Revenue', 'Formats metrics given longname');
  assert.equal(metricFormat(null, 'bar'), 'bar', 'Returns long name in case metric is not available');
  assert.equal(metricFormat({ metric: 'foo' }), '--', 'Renders -- if longname is not given');
  assert.equal(
    metricFormat({ metric: { name: 'foo', longName: 'Foo' }, parameters: { p: 1 } }, 'Bar'),
    'Bar (1)',
