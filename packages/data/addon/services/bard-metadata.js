...
export default class BardMetadataService extends Service {
  ...
  async loadMetadata(options = {}) {
    ...
    if (!this.loadedDataSources.includes(dataSource)) {
      ...
      // Send a separate request for metric functions if necessary and normalize
      metadata.metricFunctions = await this._getMetricFunctionMetadata(metadata.metricFunctions, dataSource, options);
  ...
  /**
   * @private
   * @method _getMetricFunctionMetadata - If metricFunctions are provided in an endpoint, fetch them and normalize them
   * @param {Object|null} metricFunctionMetadata - metricFunctions generated by serializer or null if provided by api
   * @param {String} dataSource - datasource name
   * @param {Object} options - sent with adapter request
   */
  async _getMetricFunctionMetadata(metricFunctionMetadata, dataSource, options) {
    let normalized = metricFunctionMetadata;
    if (normalized === null) {
