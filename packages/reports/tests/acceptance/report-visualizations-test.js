...
test('Table column sort', function(assert) {
  ...
  andThen(() => {
    //first parameter
    assert.notOk(find('.table-header-cell.metric:contains(Platform Revenue (USD)) .navi-table-sort-icon').hasClass('navi-table-sort-icon--desc'),
      'Metric with first parameter is not sorted desc');

    assert.notOk(find('.table-header-cell.metric:contains(Platform Revenue (USD)) .navi-table-sort-icon').hasClass('navi-table-sort-icon--asc'),
      'Metric with first parameter is not sorted asc');

    //second parameter
    assert.notOk(find('.table-header-cell.metric:contains(Platform Revenue (EUR)) .navi-table-sort-icon').hasClass('navi-table-sort-icon--desc'),
      'Metric with second parameter is not sorted desc');

    assert.notOk(find('.table-header-cell.metric:contains(Platform Revenue (EUR)) .navi-table-sort-icon').hasClass('navi-table-sort-icon--asc'),
  ...
  andThen(() => {
    //first parameter
    assert.ok(find('.table-header-cell.metric:contains(Platform Revenue (USD)) .navi-table-sort-icon').hasClass('navi-table-sort-icon--desc'),
  ...
  andThen(() => {
    assert.ok(find('.navi-modal__input').val().includes('sort=dateTime%7Casc%2CplatformRevenue(currency%3DUSD)%7Cdesc'),
      'API query contains sorting by metric with first parameter desc after first sort click');

    assert.notOk(find('.navi-modal__input').val().includes('sort=dateTime%7Casc%2CplatformRevenue(currency%3DEUR)'),
  ...
  andThen(() => {
    ...
    assert.ok(find('.table-header-cell.metric:contains(Platform Revenue (USD)) .navi-table-sort-icon').hasClass('navi-table-sort-icon--asc'),
  ...
  click('.navi-report__copy-api-btn .get-api__btn');
  andThen(() => {
    assert.ok(find('.navi-modal__input').val().includes('sort=dateTime%7Casc%2CplatformRevenue(currency%3DUSD)%7Casc'),
  ...
  click('.navi-report__copy-api-btn .get-api__btn');
  andThen(() => {
    assert.notOk(find('.navi-modal__input').val().includes('sort=dateTime%7Casc%2CplatformRevenue(currency%3DUSD)'),
  ...
  andThen(() => {
    ...
    //second parameter
    assert.ok(find('.table-header-cell.metric:contains(Platform Revenue (EUR)) .navi-table-sort-icon').hasClass('navi-table-sort-icon--desc'),
      'Metric with first parameter is sorted desc after sorting by both parameters');
  });
