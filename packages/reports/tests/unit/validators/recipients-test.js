...
test('validate recipients', function(assert) {
  ...
  assert.equal(Validator.validate(emptyArr, { invalidEmailMsg: badEmails, noRecipientsMsg: emptyText }),
    'There must be at least one valid email address',
    'Array is empty');

  assert.equal(Validator.validate(badArray0, { invalidEmailMsg: badEmails, noRecipientsMsg: emptyText }),
    'There must be at least one valid email address',
    'Array has an empty string');

  assert.equal(Validator.validate(badArray1, { invalidEmailMsg: badEmails, noRecipientsMsg: emptyText }),
  ...
  assert.equal(Validator.validate(regArray, { invalidEmailMsg: messageText, noRecipientsMsg: emptyText }),
    true,
