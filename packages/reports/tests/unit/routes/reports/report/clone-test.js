...
test('_cloneReport', function(assert) {
  ...
  return Ember.run(() => {
    ...
    assert.equal(clonedReport.title,
      `Copy of ${originalReport.title}`,
      'Cloned report title is "Copy of" + original title');

    assert.equal(clonedReport.request.responseFormat,
      'csv',
      'Response format is defaulted to csv, regardless of original report');

    assert.equal(clonedReport.author,
      mockAuthor,
      'Current user is the author of the cloned report');
  });
});
