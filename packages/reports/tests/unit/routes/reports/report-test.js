...
test('model hook', function(assert) {
  ...
  return route.model({ reportId: 1 }).then(model => {
    assert.equal(model.id,
      1,
      'Route model looks up report based on id');

    return route.model({ reportId: '123-456' }).then(model => {
      assert.equal(model.tempId,
        '123-456',
        'Route can find reports based on temp id');
    });
  });
});
...
test('_findByTempId', function(assert) {
  ...
  assert.equal(route._findByTempId(2).tempId,
    2,
    'Reports can be found by temp id');

  assert.equal(route._findByTempId(50),
    undefined,
    'Undefined is returned when no report has the given temp id');
});
...
test('_defaultVisualization', function(assert) {
  ...
  let mockReport = {
      ...
      route = this.subject({
        store: {
          createFragment: (type) =>  { return { type }; }
        }
      });

  assert.deepEqual(route._defaultVisualization(mockReport).visualization,
    { type: 'table' },
...
test('revertChanges action', function(assert) {
  ...
  let mockReport = {
        rollbackAttributes() {
          assert.ok(true,
            'Report model is asked to rollback');
        }
      },
      route = this.subject();
...
test('deactivate method', function(assert) {
  ...
  let mockReport = {
        ...
        rollbackAttributes() {
          assert.ok(true,
            'Route model is asked to rollback');
        }
      },
      route = this.subject({
...
test('updateTitle action', function(assert) {
  ...
  assert.equal(get(route, 'currentModel.title'),
    'Default Title',
    '`Default Title` is the current report title');
