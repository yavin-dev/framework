...
moduleForComponent('navi-date-picker', 'Unit | Component | Navi Date Picker', {
  unit: true,
  needs: [
    'component:bootstrap-datepicker-inline'
  ]
});
...
test('_isNewDateValue', function(assert){
  ...
  assert.equal(component._isNewDateValue(testDate1),
    false,
    'Date is not the same when function was never called');

  assert.equal(component._isNewDateValue(testDate1),
    true,
    'Date is the same when given the same value twice in a row');

  assert.equal(component._isNewDateValue(testDate2),
    false,
