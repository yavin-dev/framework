...
test('Model using the Interval Fragment', function(assert) {
  ...
  Ember.run(() => {
    /* == Getter Method == */
    assert.ok(mockModel.get('intervals.firstObject.interval').isEqual(
      new Interval(new Duration('P7D'), endDate)),
  ...
  assert.ok(mockModel.get('intervals.firstObject.interval').isEqual(interval),
    'The interval property is as set using the setter');

  /* == Serialize == */
  assert.deepEqual(mockModel.serialize().data.attributes.intervals,
    [
      {
...
test('Interval Validations', function(assert) {
  ...
  let interval = Ember.run(() => Store.peekRecord('fragments-mock', 1).get('intervals.firstObject'));
  ...
  return interval.validate().then(({ validations }) => {
    ...
    return interval.validate().then(({ validations }) => {
      ...
      assert.equal(validations.get('messages').length,
        1,
        'Undefined interval has one validation error');
      ...
      return interval.validate().then(({ validations }) => {
        ...
        assert.equal(validations.get('messages').length,
          1,
          'Descending interval has one validation error');
