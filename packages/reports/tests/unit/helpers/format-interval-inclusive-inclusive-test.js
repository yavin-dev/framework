import {formatInterval, formatDurationFromCurrent, formatDateRange} from 'navi-reports/helpers/format-interval-inclusive-inclusive';
...
module('Unit | Helper | format interval inclusive inclusive');


test('Undefined interval and time period', function(assert) {
  assert.expect(2);

  assert.equal(formatInterval([moment(new Interval('current', 'current')), null]),
    '',
    'No time period returns empty string');

  assert.equal(formatInterval([null, 'week']),
    '',
    'No interval returns empty string');
});
...
test('Formatting rolling window', function(assert) {
  ...
  assert.equal(formattedString,
    'Last 4 Weeks',
    'Interval was converted to week rolling window');
  ...
  assert.equal(formattedString,
    'Last 2 Quarters',
    'Interval was converted to quarter rolling window');
});
...
test('Checking Current', function(assert) {
  ...
  assert.equal(formattedString,
    'Current Month',
    'Interval was converted to Current Month');

  formattedString = formatInterval([interval, 'quarter']);

  assert.equal(formattedString,
    'Current Quarter',
    'Interval was converted to Current Quarter');
});

test('formatDurationFromCurrent', function(assert) {
  assert.expect(7);

  assert.equal(formatDurationFromCurrent(),
    '',
    'No duration returns empty string');

  assert.equal(formatDurationFromCurrent(new Duration('P4W')),
    'Last 4 Weeks',
    'Last is added to humanized duration');

  assert.equal(formatDurationFromCurrent(new Duration('P9M'), 'quarter'),
    'Last 3 Quarters',
    'Quarter is formatted correctly when in terms of a month duration');

  assert.equal(formatDurationFromCurrent(Duration.all()),
    'All',
    'All duration is kept as All');

  assert.equal(formatDurationFromCurrent(new Duration('P1D')),
    'Last Day',
    '1 day ago becomes Last Day');

  assert.equal(formatDurationFromCurrent(new Duration('P1W')),
    'Last Week',
    'Value of 1 is removed when formatting');

  assert.equal(formatDurationFromCurrent(new Duration('P3M'), 'quarter'),
    'Last Quarter',
...
test('formatDurationFromCurrent - quarter not in terms of months', function(assert) {
  ...
  assert.throws(() => formatDurationFromCurrent(new Duration('P3W'), 'quarter'),
...
test('formatDateRange - Undefined start and end', function(assert) {
  ...
  assert.throws(() => { formatDateRange(); },
    error,
    'Providing no arguments throws an error');

  assert.throws(() => { formatDateRange(moment(), null, null); },
    error,
    'Providing valid start, but no end throws an error');

  assert.throws(() => { formatDateRange(null, moment(), null); },
    error,
    'Providing valid end, but no start throws an error');

  assert.throws(() => { formatDateRange(moment(), moment(), null); },
    error,
...
test('formatDateRange - Formatting start and end', function(assert) {
  ...
  assert.equal(formattedString,
    'Sep 14, 2014 - Oct 14, 2014',
    'Given date range was correctly formatted');
