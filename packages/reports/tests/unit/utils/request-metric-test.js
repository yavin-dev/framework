...
module('Unit | Utils | request metric');
...
test('getSelectedMetricsOfBase', function(assert) {
  ...
  assert.deepEqual(arr(getSelectedMetricsOfBase({name: 'foo'}, request)).mapBy('canonicalName'),
    [ 'foo-metric1', 'foo-metric2' ],
    'getSelectedMetricsOfBase returns all the metrics in request with the baseMetric type `foo`');

  assert.deepEqual(getSelectedMetricsOfBase({name: 'foo'}, {metrics:[]}),
    [],
...
test('getFilteredMetricsOfBase', function(assert) {
  ...
  assert.deepEqual(arr(getFilteredMetricsOfBase({name: 'foo'}, request)).mapBy('metric.canonicalName'),
    [ 'foo-metric1', 'foo-metric2' ],
    'getFilteredMetricsOfBase returns all the havings in request with the baseMetric type `foo`');

  assert.deepEqual(getFilteredMetricsOfBase({name: 'foo'}, {having:[]}),
    [],
...
test('getFilteredMetricsOfBase', function(assert) {
  ...
  assert.deepEqual(arr(getUnfilteredMetricsOfBase({name: 'foo'}, request)).mapBy('canonicalName'),
    [ 'foo-metric2' ],
    'getUnfilteredMetricsOfBase returns all the metrics in request that are not filtered with baseMetric type `foo`');

  assert.deepEqual(getUnfilteredMetricsOfBase({name: 'foo'}, {having:[]}),
    [],
