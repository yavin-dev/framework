...
module('Unit | Utils | Enums - Filter Operations');
...
test('all', function(assert) {
  ...
  assert.deepEqual(allFilterOperations,
    expectedOperations,
    'all returns the expected array of response formats');
});
...
test('getById', function(assert){
  ...
  assert.equal(responseFormat,
    allFilterOperations[0],
    'getById returned expected object with id in');

  responseFormat = FilterOperations.getById('notin');
  assert.equal(responseFormat,
    allFilterOperations[1],
    'getById returned expected object with id notin');
