...
moduleForComponent('power-select-bulk-import-trigger', 'Integration | Component | power select bulk import trigger', {
  ...
  beforeEach() {
    ...
    return this.container.lookup('service:bard-metadata').loadMetadata().then(() => {
      return this.container.lookup('service:bard-dimensions').all('property').then(allProperties => {
        this.setProperties({
...
test('it renders', function(assert) {
  ...
  assert.ok($('.ember-power-select-multiple-options').is(':visible'),
...
test('paste to trigger bulk import', function(assert) {
  ...
  assert.notOk($('.dimension-bulk-import').is(':visible'),
  ...
  assert.ok($('.dimension-bulk-import').is(':visible'),
    'Bulk import modal opens when pasting text with a ","');

  return wait().then(() => {
    let validPills = $('.id-container:first .item');
    assert.deepEqual(validPills.map(function() { return this.childNodes[0].wholeText.trim(); }).get(),
      [
        'Property 1 (114)',
        'Property 4 (101272)'
      ],
    ...
    assert.deepEqual(invalidPills.map(function() { return this.textContent.trim(); }).get(),
      ['78787'],
...
test('importing dimensions', function(assert) {
  ...
  const selectedValues = () => $('.selected-dim-id').map(function() { return this.textContent.trim(); }).get();
  ...
  return wait().then(() => {
    ...
    assert.deepEqual(selectedValues(),
      ...
      'Only valid pasted ids are imported');
    ...
    return wait().then(() => {
      ...
      assert.deepEqual(selectedValues(),
        ...
        'Duplicate ids are not imported twice');
...
test('trying to import invalid values', function(assert) {
  ...
  return wait().then(() => {
    ...
    assert.deepEqual(selectedValues(),
      [],
      'No ids are imported when none are valid');
  });
});
