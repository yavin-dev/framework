...
test('metric label visualization selector is available on single metric, single time bucket, no dimensions', function(assert) {
  ...
  return wait().then(() => {
    ...
    assert.ok(this.$('.report-view__visualization-option:contains(Data Table)').is(':visible'),
      'Table Selector is visible');

    assert.ok(this.$('.report-view__visualization-option:contains(Metric Label)').is(':visible'),
...
test('no data', function(assert) {
  ...
  return wait().then(() => {
    ...
    return wait().then(() => {
      ...
      assert.equal(this.$('.report-view__visualization-no-results').text().trim(),
