...
test('component is rendered in trimmed mode', function(assert) {
  ...
  assert.equal(this.$('.items-container.trimmed').length,
    1,
    'items container has "trimmed" class');

  assert.equal(this.$('a:contains("Show more")').length,
    1,
...
test('component displays more items on clicking show more link', function(assert) {
  ...
  assert.equal(this.$('.items-container.show-all').length,
    1,
...
test('component is rendered in show all state', function(assert) {
  ...
  assert.equal(this.$('.items-container.show-all').length,
    1,
    'items container has "show-all" class');

  assert.equal(this.$('a:contains("Show more")').length,
    0,
    'Show more link is not visible when trim flag is false');
});
...
test('show more link is not visible when items are within max height', function(assert) {
  ...
  assert.equal(this.$('a:contains("Show more")').length,
    0,
    'Show more link is not visible initially');
  ...
  assert.equal(this.$('a:contains("Show more")').length,
    1,
    'Show more link is visible after items change');
});
...
test('scrolling to the bottom loads more elements', function(assert) {
  ...
  assert.equal($('.mock-item').length,
    25,
    '25 items are shown before scrolling');
  ...
  return wait().then(() => {
    assert.equal(this.$('.mock-item').length,
      50,
      '50 items are shown after scrolling');
  });
});
...
test('updating items or perPage will cause _itemsToRender to update', function(assert) {
  ...
  assert.equal($('.item').length,
    5,
    '_itemsToRender is initialized correctly');

  this.set('items', _buildItemsArray(10, 'This is Foo Object'));

  assert.equal($('.item').length,
    10,
    'updating with new items array causes _itemsToRender to be updated correctly');
  ...
  assert.equal($('.item').length,
    9,
    'updating existing items array causes _itemsToRender to be updated correctly');

  this.set('perPage', 4);

  assert.equal($('.item').length,
    4,
    'updating perPage causes _itemsToRender to be updated correctly');
});
