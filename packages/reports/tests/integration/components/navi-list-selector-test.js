...
test('it renders', function(assert) {
  ...
  assert.ok(this.$('.navi-list-selector').is(':visible'),
    'The navi-list-selector component is rendered');

  assert.equal(this.$('.navi-list-selector__title').text().trim(),
    'Items',
    'The navi-list-selector component renders a title');

  assert.equal(this.$('.navi-list-selector__search-input').attr('placeholder'),
    'Search Items',
    'The navi-list-selector search bar has the title in the placeholder');

  assert.ok(this.$('.navi-list-selector__show-link').is(':visible'),
    'The navi-list-selector`s show all/show selected link is rendered');

  assert.ok(this.$('.navi-list-selector__search').is(':visible'),
    'The navi-list-selector`s search bar is rendered');
});
...
test('show all/show selected', function(assert) {
  ...
  assert.ok(Ember.isEmpty(this.$('.test-item__filtered')),
    'the boolean `areItemsFiltered` is falsy when the item list is unfiltered');

  assert.deepEqual(this.$('.test-item').toArray().map(el => $(el).text().trim()),[
    'foo',
    'bar',
    'baz'
  ...
  assert.deepEqual(this.$('.test-item__filtered').toArray().map(el => $(el).text().trim()),[
    'foo'
  ...
  assert.equal(this.$('.navi-list-selector__content--error').text().trim(),
    'No items selected',
