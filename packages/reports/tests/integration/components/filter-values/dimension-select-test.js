...
test('it renders', function(assert) {
  ...
  clickTrigger();
  return wait().then(() => {
    let selectedValueText = this.$('.ember-power-select-multiple-option span:nth-of-type(2)').map(function() { return $(this).text().trim(); }).get(),
    ...
    let optionText = $('.ember-power-select-option').map(function() { return $(this).text().trim(); }).get(),
...
test('changing values', function(assert) {
  ...
  this.onUpdateFilter = (changeSet) => {
    assert.deepEqual(changeSet.rawValues,
...
test('error state', function(assert) {
  ...
  this.render(hbs`{{filter-values/dimension-select filter=filter}}`);
  assert.notOk(this.$('.filter-values--dimension-select--error').is(':visible'),
