...
test('show-all component renders', function(assert) {
  ...
  assert.equal($('.navi-modal-header .primary-header').text().trim(),
    'Included properties',
    'component has main header text based on dimension filter');

  assert.equal($('.navi-modal-header .secondary-header').text().trim(),
    'The properties listed below have been included',
    'component has secondary header text based on dimension filter');

  assert.equal($('.show-all .valid-id-count').text().replace(/\s+/g, " ").trim(),
    '10 properties',
  ...
  assert.deepEqual(pills.map(function() { return this.childNodes[0].wholeText.trim(); }).get(),
    expectedResult,
    'component displays appropriate filter values as expected');

  assert.equal($('.show-all .btn').text().trim(),
    'Done',
...
test('behaviour of done button', function(assert) {
  ...
  assert.equal($('button:contains("Done")').length,
    1,
    'Done button is visible before updating filter values');
...
test('behaviour of cancel button', function(assert) {
  ...
  assert.equal($('button:contains("Cancel")').length,
    0,
  ...
  assert.equal($('button:contains("Cancel")').length,
    1,
    'Cancel button is visible after updating filter values');
...
test('updating filter values', function(assert) {
  ...
  assert.equal($('button:contains("Update")').length,
    0,
    'Update button is not visible before updating filter values');

  assert.equal($('.items-count').text().trim(),
    '10',
  ...
  assert.equal($('button:contains("Update")').length,
    1,
    'Update button is visible after updating filter values');
