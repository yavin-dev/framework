 ...
 *
 * Code adapted from https://github.com/ember-animation/ember-animated/blob/v0.10.1/addon/transitions/move-over.js and modified to yield promises
 */
import { Move } from 'ember-animated/motions/move';
import follow from 'ember-animated/motions/follow';
...
export default function* moveOver(dimension, direction, context) {
  let { position, size, startTranslatedBy, endTranslatedBy } = normalize(dimension, direction);
  ...
  if (context.insertedSprites.length) {
    ...
    if (context.removedSprites.length > 0) {
      ...
      for (let i = 1; i < context.removedSprites.length; i++) {
        yield follow(context.removedSprites[i], { source: move });
      }
      yield follow(context.insertedSprites[0], { source: move });
    } else {
    ...
    yield move.run();
    const promises = context.removedSprites.map((sprite) => {
