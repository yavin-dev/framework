...
test('Add to dashboard button is hidden when there are unrun request changes', function(assert) {
   ...
   */
  click(
    '.checkbox-selector--metric .grouped-list__item:contains(Ad Clicks) .grouped-list__item-label'
  );
  click(
    '.checkbox-selector--metric .grouped-list__item:contains(Nav Link Clicks) .grouped-list__item-label'
  );
  andThen(() => {
    assert.notOk(
      find('.navi-report__action:contains("Add to Dashboard")').is(':visible'),
