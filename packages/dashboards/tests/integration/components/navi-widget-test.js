...
test('it renders', function(assert) {
  ...
  assert.ok(
    this.$('.loader-container').is(':visible'),
  ...
  assert.ok(
    this.$('.error-container').is(':visible'),
...
test('visualization', function(assert) {
      ...
      didInsertElement() {
        ...
        this.get('containerComponent')
          ...
          .on('resizestop', () => {
            assert.ok(
              true,
              'visualization can listen to resize events on containerComponent property'
            );
          });
      }
    })
  ...
  return wait().then(() => {
    assert.ok(
      this.$('.test-visualization').is(':visible'),
...
test('delete action visibility', function(assert) {
  ...
  assert.ok(
    !!this.$('.navi-widget__delete-btn').length,
    'Delete action is visible when user can edit'
  );

  this.set('canEdit', false);
  assert.notOk(
    !!this.$('.widget-actions .delete').length,
