...
moduleForComponent('navi-dashboard', 'Integration | Component | navi dashboard', {
  ...
  beforeEach() {
    ...
    return this.container.lookup('service:user').findUser().then(() => {
      ...
      return this.container.lookup('service:bard-metadata').loadMetadata().then(() => {
        ...
        Ember.run(() => {
          ...
          store.push({ data: [{ id: 1, type: 'dashboard-widget' }, { id: 2, type: 'dashboard-widget' }]});
        });
      });
    });
  },
...
test('it renders', function(assert) {
  ...
  return wait().then(() => {
    ...
    assert.equal(this.$('.page-title').text().trim(),
      this.get('dashboardModel.title'),
      'Component renders header with dashboard title');

    assert.equal(this.$('.grid-stack .grid-stack-item').length,
      2,
      'Component renders a grid-stack-item for each widget, ignoring ones that do not exist');

    assert.equal(this.$('.grid-stack .grid-stack-item:eq(1)').attr('data-gs-x'),
      3,
      'Widget x position is based on layout');

    assert.equal(this.$('.grid-stack .grid-stack-item:eq(1)').attr('data-gs-width'),
      6,
...
test('dashboard export', function(assert) {
  ...
  return wait().then(() => {
    ...
    this.render(hbs`{{navi-dashboard dashboard=dashboardModel}}`);
    assert.notOk($('.action.export').is(':visible'),
