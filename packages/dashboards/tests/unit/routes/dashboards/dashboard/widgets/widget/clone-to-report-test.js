...
moduleFor('route:dashboards/dashboard/widgets/widget/clone-to-report', 'Unit | Route | dashboards/dashboard/widgets/widget/clone-to-report', {
  needs: [
    'model:user',
    'model:report',
    'model:delivery-rule',
...
test('_cloneToReport', function(assert) {
  ...
  return Ember.run(() => {
    ...
    assert.equal(clonedReport.get('title'),
      `Copy of ${widget.title}`,
      'Created report title is "Copy of" + original title');

    assert.equal(clonedReport.get('author.id'),
      MockAuthor.id,
      'Current user is the author of the created report');
  });
});
