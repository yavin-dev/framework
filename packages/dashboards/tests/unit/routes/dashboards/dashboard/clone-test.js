  ...
  return Ember.run(() => {
    return Route.store.findRecord('dashboard', 1).then((dashboard) => {
      ...
      return cloneDashboard.then(model => {
        ...
        assert.equal(expectedModel.title,
          CLONED_MODEL.title,
          'Expected Models title is correct');

        assert.equal(expectedModel.author,
          CLONED_MODEL.author,
          'Expected Models Author is correct');

        assert.notEqual(widgetIdinExpectedModel,
          widgetIdinDashboard1,
          'WidgetIds in expected dashboard and original dashboard is not the same');

        assert.deepEqual(expectedModel.presentation,
  ...
  return Ember.run(() => {
    return Route.store.findRecord('dashboard', 1).then((dashboard) => {
      ...
      return cloneDashboard.catch(() => {
        assert.ok(true,
          'clone dashboard fails for invalid widgets');
      });
    });
  });
});
          ...
          Route._cloneWidgets(dashboard, cloneDashboardModel).then(async widgetPromiseArray => {
            widgetPromiseArray.forEach((widgetPromise, idx) =>
              widgetPromise.then((clonedWidget) => {
                ...
                assert.equal(get(clonedWidget, 'title'),
                  oldWidgets.objectAt(idx).get('title'),
                  'Widget titles are same');

                assert.deepEqual(get(clonedWidget, 'requests.firstObject').toJSON(),
                  oldWidgets.objectAt(idx).get('requests.firstObject').toJSON(),
                  'Widget requests are same');

                assert.deepEqual(get(clonedWidget, 'visualization').toJSON(),
                  oldWidgets.objectAt(idx).get('visualization').toJSON(),
                  'Widget visualization are same');

                assert.notEqual(get(clonedWidget, 'id'),
                  oldWidgets.objectAt(idx).get('id'),
                  'Widget ids are not same');

                assert.equal(get(clonedWidget, 'dashboard.id'),
  ...
  return wait().then(() => {
    ...
    Route.replaceWith = (destinationRoute) => {
      assert.equal(destinationRoute,
        ...
        'Route redirects to dashboard/:id route ');
    };
