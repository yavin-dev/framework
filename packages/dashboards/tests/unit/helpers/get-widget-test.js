...
test('getWidget', function(assert) {
  ...
  assert.equal(getWidget.compute([2]).get('id'),
    '2',
    'widget with given id is returned');

  assert.equal(getWidget.compute([24]),
    null,
    'undefined is returned when no widget has that id');
});
