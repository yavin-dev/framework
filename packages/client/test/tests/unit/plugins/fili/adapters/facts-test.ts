...
const TestRequest: RequestV2 = {
  ...
  MockBardResponse = Response;

let Adapter: FiliFactsAdapter, clientConfig: ClientConfig;
const defaultRequestDecorator = { applyGlobalDecorators: (r: RequestV2) => r };
...
module('Unit | Plugins | Fili | Adapters | facts', function (hooks) {
  ...
  hooks.beforeEach(function (this: WithServer) {
    ...
    clientConfig.dataSources.push({ displayName: 'Test', name: 'test', type: 'bard', uri: 'test://' });
    Adapter = new FiliFactsAdapter(
      Mock()
        .config(clientConfig)
        .decorator(defaultRequestDecorator)
        .meta({ getById: () => undefined } as unknown as MetadataService)
        .build()
    );

    this.server.use(
      rest.get(`${HOST}/v1/data/table1/grain1/d1;show=id/d2;show=desc/*`, (req, res, ctx) => {
        ...
        return res(ctx.json(MockBardResponse));
      })
    );
  });
  ...
  test('_buildDateTimeParam', function (assert) {
    ...
    let originalDataEpoch = clientConfig.dataEpoch;
    // TODO: Remove once dataEpoch is not required
    clientConfig.dataEpoch = undefined;
    assert.deepEqual(
    ...
    clientConfig.dataEpoch = '1999-02-03';
    assert.deepEqual(
      Adapter._buildDateTimeParam(before),
    ...
    clientConfig.dataEpoch = originalDataEpoch;
  ...
  test('_buildURLPath', function (assert) {
    ...
    const adapter = new FiliFactsAdapter(
      Mock()
        .config(clientConfig)
        .decorator(defaultRequestDecorator)
        .meta({ getById: () => ({ hasAllGrain: false }) } as unknown as MetadataService)
        .build()
    );
    assert.throws(
      () => {
  ...
  test('fetchDataForRequest with client id options', async function (this: WithServer, assert) {
    assert.expect(2);

    // Setting up assert for default clientId
    this.server.use(
      rest.get(`${HOST}/v1/data/table1/grain1/d1;show=id/d2;show=desc/`, (req, res, ctx) => {
        assert.equal(req.headers.get('clientid'), 'UI', 'Client id is set to value given in options');
        return res(ctx.json(MockBardResponse));
      })
    );
    ...
    this.server.use(
      rest.get(`${HOST}/v1/data/table1/grain1/d1;show=id/d2;show=desc/`, (req, res, ctx) => {
        assert.equal(req.headers.get('clientid'), 'test id', 'Client id is set to value given in options');
        return res(ctx.json(MockBardResponse));
      })
    );
    await Adapter.fetchDataForRequest(TestRequest, {
  ...
  test('fetchDataForRequest with custom headers', async function (this: WithServer, assert) {
    assert.expect(2);

    this.server.use(
      rest.get(`${HOST}/v1/data/table1/grain1/d1;show=id/d2;show=desc/`, (req, res, ctx) => {
        assert.equal(req.headers.get('foo'), 'bar', 'custom header `foo` was sent with the request');

        assert.equal(req.headers.get('baz'), 'qux', 'custom header `baz` was sent with the request');
  ...
  test('fetchDataForRequest with alternative datasource', async function (this: WithServer, assert) {
    assert.expect(1);

    this.server.use(
      rest.get(`${HOST2}/v1/data/table1/grain1/d1;show=id/d2;show=desc/`, (req, res, ctx) => {
        assert.ok(req.url.toString().startsWith(HOST2), 'Alternative host was accessed');
